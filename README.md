<h1 align="center">Hi there, I'm Wahid 👋</h1>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=WahidMubarrat&label=Profile%20Views&color=0e75b6&style=flat" alt="profile views" />
  <a href="https://www.linkedin.com/in/wahid-mubarrat-25477b1b2/">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white" />
  </a>
  <a href="mailto:wahidazhar@iut-dhaka.edu">
    <img src="https://img.shields.io/badge/Email-D14836?style=flat&logo=gmail&logoColor=white" />
  </a>
</p>

### 🎓 About Me
- Software Engineering student at **Islamic University of Technology (IUT)**, expected graduation **2027**
- Building full-stack web apps — MERN stack, Next.js, FastAPI
- Exploring **AI-integrated applications** (Gemini API, RAG, agentic workflows)
- Hackathon participant — CodeSprint, bKash SUST CSE Carnival

---

### 🛠️ Tech Stack

**Languages**
<p>
  <img src="https://skillicons.dev/icons?i=java,python,c,cpp,js,ts" />
</p>

**Frontend**
<p>
  <img src="https://skillicons.dev/icons?i=react,nextjs,tailwind,vite,html,css" />
</p>

**Backend**
<p>
  <img src="https://skillicons.dev/icons?i=nodejs,express,fastapi" />
</p>

**Databases**
<p>
  <img src="https://skillicons.dev/icons?i=mongodb,postgres,mysql,neo4j" />
  <img src="https://img.shields.io/badge/Oracle-F80000?style=flat&logo=oracle&logoColor=white" />
</p>

**Tools & Platforms**
<p>
  <img src="https://skillicons.dev/icons?i=git,github,docker,figma" />
</p>

---

### 🚀 Featured Projects

| Project | Description | Tech |
|---|---|---|
| **[LeaveTracker](https://github.com/WahidMubarrat/LeaveTracker)** | Role-based leave management with 3-tier approval, OCR, and email automation | MERN, JWT, OCR |
| **[CalorieCraft](https://github.com/mustain98/calorie-craft)** | Full-stack meal planning platform with nutrition tracking | MERN, JWT, Cloudinary |
| **[Gym Management System](https://github.com/WahidMubarrat/GYM-MANAGEMENT-SYSTEM)** | Desktop gym app with PL/SQL-driven backend | Java Swing, Oracle |
| **[Token-Table](https://github.com/WahidMubarrat/SPL--1)** | University meal token system, OOP course project | Java, OOP |

---


